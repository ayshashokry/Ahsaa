define([
  ...
  function difference(
    arr1 /*new objectIds*/,
    cacheCount /*objectId cache length*/,
    hash /*objecid hash*/
  ) {
    //var start = new Date().valueOf();
  ...
  // esriConfig.defaults.io.corsEnabledServers.push("37.224.12.211");
  ...
  return declare([GraphicsLayer], {
    constructor: function (options) {
      ...
      this._showSingles = options.hasOwnProperty("showSingles")
        ? options.showSingles
        : true;
      this._zoomOnClick = options.hasOwnProperty("zoomOnClick")
        ? options.zoomOnClick
        : true;
      this._imageServerUrl = options._imageServerUrl|| window.imagesServerUrl;
      // symbol for single graphics
      //this._singleSym = options.singleSymbol || new SimpleMarkerSymbol('circle', 6, null, new Color('#888'));
      this._singleSym =
        options.singleSymbol ||
        new SimpleMarkerSymbol(
          "circle",
          16,
          new SimpleLineSymbol(
            SimpleLineSymbol.STYLE_SOLID,
            new Color([85, 125, 140, 1]),
            3
          ),
      ...
      this._useDefaultSymbol = options.hasOwnProperty("useDefaultSymbol")
        ? options.useDefaultSymbol
        : false;
      ...
      this.MODE_SNAPSHOT = options.hasOwnProperty("MODE_SNAPSHOT")
        ? options.MODE_SNAPSHOT
        : true;
      esriConfig.defaults.geometryService =window.__geometryServiceUrl__;
    ...
    _getDefaultSymbol: function (g) {
      console.log({"g in _getDefaultSymbol":g});
      ...
      if (!this._useDefaultSymbol || !rend) {
        ...
        if(Object.keys(g.attributes).includes("SITE_MAIN_ACTIVITY_FORAS")){
          if(g.attributes.SITE_MAIN_ACTIVITY_FORAS)
          pointSymbol.setUrl(`${
            this._imageServerUrl
          }/SITE_MAIN_ACTIVITY_FORAS/${g.attributes.SITE_MAIN_ACTIVITY_FORAS}.svg`)
          else pointSymbol = this._singleSym;
          return pointSymbol 
        }
        else{
          pointSymbol.setUrl(`${
            this._imageServerUrl
          }/SITE_MAIN_ACTIVITY_FORAS/${15131}.svg`)
          return pointSymbol
        }
    ...
    _getRenderedSymbol: function (feature) {
      ...
      if (attr.clusterCount === 1) {
        if (!this._useDefaultSymbol) {
          ...
          if(Object.keys(feature.attributes).includes("SITE_MAIN_ACTIVITY_FORAS")){
            if(feature.attributes.SITE_MAIN_ACTIVITY_FORAS)
            pointSymbol.setUrl(`${
              this._imageServerUrl
            }/SITE_MAIN_ACTIVITY_FORAS/${feature.attributes.SITE_MAIN_ACTIVITY_FORAS}.svg`)
            else pointSymbol = this._singleSym;
            return pointSymbol 
          }
          else{
        ...
        var rend = this._defaultRenderer;
        if (!rend) {
          // something went wrong getting default renderer
          return null;
        } else {
    ...
    _reCluster: function () {
      if (!this.suspended) {
        ...
        this._visitedExtent = this._visitedExtent
          ? this._visitedExtent.union(this._map.extent)
          : this._map.extent;
      }
    },
    ...
    _setMap: function (map) {
      ...
      this._extentChange = on.pausable(
        map,
        "extent-change",
        lang.hitch(this, "_reCluster")
      );
    ...
    _onIdsReturned: function (results) {
      var uncached = difference(
        results,
        this._objectIdCache.length,
        this._objectIdHash
      );
      ...
      if (uncached && uncached.length) {
        ...
        if (uncached.length > this._returnLimit) {
          ...
          this.queryTask
            .execute(this._query)
            .then(lang.hitch(this, "_onFeaturesReturned"), this._onError);
        }
    ...
    onClick: function (e) {
      ...
      if (e.graphic.attributes.clusterCount === 1) {
        // this._showClickedCluster(true);    //to remove click event on single features to show popup ==> comment this line 
        ...
        //  //to remove click event on single features to show popup ==> comment these next line 
        ...
        //   this._map.infoWindow.show(e.graphic.geometry);
        // }
        // to remove click event on single features to show popup ==>till here
      }
    ...
    _clusterAddPoint: function (feature, p, cluster) {
      // debugger
      p.attributes = {...feature.attributes, layerName:this._layerName}
      ...
      feature.attributes.clusterId = p.attributes.clusterId =
        cluster.attributes.clusterId;
    ...
    _findCluster: function (id) {
      var cg = arrayUtils.filter(this.graphics, function (g) {
        return !g.symbol && g.attributes.clusterId === id;
      });
    },
    ...
    _updateClusterGeometry: function (c) {
      ...
      if (cg.length === 1) {
        ...
        console.log(
          "didn not find exactly one cluster geometry to update: ",
          cg
        );
      }
    },
    ...
    _getNormalizedExtentsPolygon: function () {
      ...
      var normalizedExtentPolygons = arrayUtils.map(
        normalizedExtents,
        function (extent) {
