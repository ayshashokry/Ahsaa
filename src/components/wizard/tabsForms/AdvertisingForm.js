...
function AdvertisingForm(props) {
    ...
    if (props.isOneLand) {
      if (props.data) {
        Object.keys(adBoardData).forEach(item => {
          if (item === "GROUP_BOARD_PERPDATE")
            adBoardData[item] = new Date(props.data[item]).getTime();
          else if (item === "BOARD_TYPE" && item !== "")
            adBoardData[item] = props.data[item];
          else if (item === "LIGHT_STATUS" && item !== "")
            adBoardData[item] = props.data[item];
          adBoardData[item] = props.data[item];
        });
      } else adBoardData["GROUP_CODE"] = props.mainData.GROUP_CODE;
    } else {
      if (props.tblData && props.tblData.adBoardsDataAfter) {
        Object.keys(adBoardData).forEach(item => {
          if (item === "GROUP_BOARD_PERPDATE")
            adBoardData[item] = new Date(
              props.tblData.adBoardsDataAfter[item]
            ).getTime();

          adBoardData[item] = props.tblData.adBoardsDataAfter[item];
        });
      } else {
        adBoardData["GROUP_CODE"] = !props.isOneLand
          ? props.selectedAdBoards[0]
          : props.mainData.GROUP_CODE;
      }
    }
  ...
  const onChange = e => {
    ...
    if (e.target.name === "GROUP_BOARD_PERPDATE") console.log(e);
  ...
  const handleChangeDate = (date, e) => {
    ...
    onSetForm({
      ...formData,
      ["GROUP_BOARD_PERPDATE"]: new Date(date._d).getTime()
    });
  };
  ...
  const renderDomainSelect = fieldname => {
    ...
    var domain = fields[layername].find(field => field.name == fieldname).domain
      .codedValues;
    ...
    if (validationCodition) {
      ...
      if (!props.isOneLand) {
        ...
        Object.entries(formData).forEach(item => {
          ...
          else if (item[0] === "BOARD_TYPE" && item[1] !== "")
            dataOfBoards[item[0]] = item[1];
          else if (item[0] === "LIGHT_STATUS" && item[1] !== "")
            dataOfBoards[item[0]] = item[1];
        });
        for (let i = 0; i < props.selectedAdBoardsIds.length; i++) {
          const adID = props.selectedAdBoardsIds[i];
          props.editAttributes(adID, "adBoardsDataAfter", dataOfBoards);
          let currentFeature = tableSettingsClone.result.find(
            f => f.id === adID
          );
          currentFeature.isCompletedFilled.tblData.bool = true;
          props.editAttributes(adID, "isCompletedFilled", {
            ...currentFeature.isCompletedFilled,
            tblData: { ...currentFeature.isCompletedFilled.tblData, bool: true }
          });
        }
      } else {
        let adBoardsRows = tableSettingsClone.result
          .filter(
            feat => feat.layername.toLocaleLowerCase() !== "invest_site_polygon"
          )
          .filter(
            feat => feat.fieldsForEdit.GROUP_CODE === formData.GROUP_CODE
          );
        let dataOfBoards = {};
        Object.entries(formData).forEach(item => {
          if (item[1]) dataOfBoards[item[0]] = item[1];
        ...
        adBoardsRows.forEach(feat => {
          props.editAttributes(
            feat.fieldsForEdit.SITE_GEOSPATIAL_ID,
            "adBoardsDataAfter",
            dataOfBoards
          );
    ...
    <>
      ...
      <Form
        ...
        initialValues={{
          ...
          LIGHT_STATUS:
            formData.LIGHT_STATUS || formData.LIGHT_STATUS == 0
              ? formData.LIGHT_STATUS
              : null,
          GROUP_BOARD_PERPDATE: formData.GROUP_BOARD_PERPDATE
          ...
          >
            ...
            >
              <DatePicker
                ...
                // value={formData.GROUP_BOARD_PERPDATE?formData.GROUP_BOARD_PERPDATE:undefined}
