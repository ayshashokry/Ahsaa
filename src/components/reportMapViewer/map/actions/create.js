import { LoadModules } from "../../../common/mapviewer";
...
export const createMap = (mapDiv, settings, layers) => {
    return LoadModules(['esri/map', 'esri/layers/ArcGISDynamicMapServiceLayer',"esri/layers/GraphicsLayer", "esri/layers/FeatureLayer", "esri/layers/ArcGISTiledMapServiceLayer"])
    .then(([Map, ArcGISDynamicMapServiceLayer,GraphicsLayer,FeatureLayer,ArcGISTiledMapServiceLayer]) => {
        ...
        layers.forEach(function (layer, key) {
            map.addLayer(createLayer(layer, {ArcGISDynamicMapServiceLayer,GraphicsLayer,FeatureLayer,ArcGISTiledMapServiceLayer}));
        });
        return map;
    });
}
