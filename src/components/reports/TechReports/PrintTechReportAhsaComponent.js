...
class PrintAhsaComponent extends Component {
  ...
  getFeaturesInfo(mapInfo, dataInfo) {
    ...
    let where = localStorage.getItem("techReportQueryCondition");
    ...
    return new Promise((resolve, reject) => {
      features.forEach((feature) => {
        ...
        queryTask({
          ...
          outFields: ["CREATED_DATE,SITE_GEOSPATIAL_ID"],
          orderByFields: ["CREATED_DATE DESC"],
          resultRecordCount: "1",
          callbackResult: (data) => {
            if(data.features.length){
              let latestDate = data.features[0].attributes["CREATED_DATE"];
          let timeStamp = new Date(latestDate);
          let datestring =
          ...
          let whereCondition = `CREATED_DATE>=timestamp '${datestring}' AND SITE_GEOSPATIAL_ID = ${feature.attributes.SITE_GEOSPATIAL_ID}`;
            queryTask({
              url: window.ahsaReportMapUrl + "/" + buildingDetailsTbl.id,
              where:whereCondition,
              outFields: layersSetting["TBL_BUILD_DETAILS"].outFields,
              callbackResult: (dataFinal) => {
                getFeatureDomainName(
                  dataFinal.features,
  ...
  fetchContractInfo =async(geoSpatialIDsAsStr)=>{
    let geoSpatialIDsArr = geoSpatialIDsAsStr.split(',');
    let contractDataForAll = [];
    geoSpatialIDsArr.forEach(async(id,index)=>{
      let contractInfo = await axios.get(window.API_URL+`contract-info?spatial_id=${Number(id)}`,{
        headers: {
  ...
  componentDidMount() {
    ...
    let geoSpatialIDs = localStorage.getItem("techReportQueryCondition")
    if(geoSpatialIDs){
    let geoIDsAsString = geoSpatialIDs.slice(geoSpatialIDs.indexOf('(')+1,geoSpatialIDs.length-1);
      
      this.fetchContractInfo(geoIDsAsString)
    }
...
export default withRouter(connect(mapStateToProps)(PrintAhsaComponent))
